...
namespace Syncfusion.Blazor.Toolkit.Internal
{
    ...
    internal static class HijriParser
    {
        ...
        /// <summary>
        /// Modified Julian Date (MJD) correction table for Hijri calendar alignment.
        /// These values represent the MJD for the first day of each Hijri month,
        /// used to ensure accurate conversion between calendars accounting for lunar cycle variations.
        /// </summary>
        private static readonly int[] _dateCorrection = [ 28607, 28636, 28665, 28695, 28724, 28754, 28783, 28813, 28843, 28872, 28901, 28931, 28960, 28990,
        ...
        /// <summary>
        /// Converts a Gregorian <see cref="DateTime"/> to the corresponding Hijri date
        /// using the internal astronomical algorithm and the correction table.
        /// </summary>
        /// <param name="gregorianDate">The Gregorian date to convert.</param>
        /// <returns>A <see cref="HijriDate"/> containing the computed Hijri year, month and day.</returns>
        internal static HijriDate ToHijriDate(DateTime gregorianDate)
        {
            ...
            // Adjust year and month for Julian date calculation (astronomical algorithm)
            AdjustYearAndMonthForJulianCalculation(ref year, ref month);

            // Convert Gregorian date to Julian Day Number
            int julianDayNumber = CalculateJulianDayNumber(year, month, day);

            // Calculate Modified Julian Date for Hijri conversion
            int modifiedJulianDate = julianDayNumber - MODIFIED_JULIAN_DATE_OFFSET;

            // Perform primary Hijri calculation using astronomical algorithm
            HijriDateComponents primaryHijri = CalculatePrimaryHijriDate(julianDayNumber);

            // Apply correction table for accurate Hijri date
            HijriDateComponents correctedHijri = ApplyCorrectionTable(modifiedJulianDate);

            // Select the most accurate result based on day validation
            return SelectMostAccurateHijriDate(correctedHijri, primaryHijri);
        }
        /// <summary>
        /// Converts a Hijri date (year, month, day) to the corresponding Gregorian
        /// <see cref="DateTime"/> using the correction table and Julian/Gregorian conversions.
        /// </summary>
        ...
        internal static DateTime ToGregorian(HijriDate hijriDate)
        {
            ...
            // Calculate lunar number from Hijri date components
            int lunarNumber = CalculateLunarNumberFromHijri(hijriDate);

            // Get Modified Julian Day Number using correction table
            int modifiedJulianDayNumber = CalculateModifiedJulianDayNumber(hijriDate, lunarNumber);

            // Convert to Julian Date
            int julianDate = modifiedJulianDayNumber + MODIFIED_JULIAN_DATE_OFFSET;

            // Convert Julian Date to Gregorian calendar
            return ConvertJulianToGregorian(julianDate);
        }

        #region Private Helper Methods - Gregorian to Hijri Conversion

        /// <summary>
        /// Adjusts January and February so they are treated as months 13 and 14
        /// of the previous year. This modification is required by the Julian
        /// day number calculation algorithm.
        /// </summary>
        /// <param name="year">The year to adjust (passed by reference).</param>
        /// <param name="month">The month to adjust (passed by reference).</param>
        private static void AdjustYearAndMonthForJulianCalculation(ref int year, ref int month)
        {
        ...
        /// <summary>
        /// Computes the Julian Day Number (JDN) from Gregorian date values
        /// using a standard astronomical formula adapted for integer arithmetic.
        /// </summary>
        /// <param name="year">The adjusted year used in the Julian calculation.</param>
        /// <param name="month">The adjusted month used in the Julian calculation.</param>
        /// <param name="day">The adjusted day of the month.</param>
        /// <returns>The Julian Day Number corresponding to the supplied Gregorian date value.</returns>
        private static int CalculateJulianDayNumber(int year, int month, int day)
        {
            int centuryPrefix = (int)Math.Floor(year / 100.0);
            int julianOffset = centuryPrefix - (int)Math.Floor(centuryPrefix / 4.0) - 2;
            return (int)Math.Floor(GREGORIAN_YEAR_DAYS * (year + YEAR_BASE_OFFSET)) + (int)Math.Floor(MONTH_APPROXIMATION * (month + 1))
                   + day - julianOffset - JULIAN_CALENDAR_ADJUSTMENT;
        }

        /// <summary>
        /// Estimates Hijri year, month and day from a Julian Day Number using the
        /// arithmetic Islamic calendar approximation based on a 30-year cycle.
        /// </summary>
        /// <param name="julianDayNumber">The Julian Day Number used as input for the calculation.</param>
        /// <returns>A <see cref="HijriDateComponents"/> containing year, month and day computed by the algorithm.</returns>
        private static HijriDateComponents CalculatePrimaryHijriDate(int julianDayNumber)
        {
            double islamicYearLength = HIJRI_CYCLE_DAYS / HIJRI_CYCLE_YEARS;
            int daysSinceEpoch = julianDayNumber - HIJRI_EPOCH_OFFSET;
            // Calculate Islamic cycle and position within cycle
            int cycle = (int)Math.Floor(daysSinceEpoch / HIJRI_CYCLE_DAYS);
            int remainingDays = daysSinceEpoch - (int)(HIJRI_CYCLE_DAYS * cycle);
            // Calculate year within cycle
            int yearInCycle = (int)Math.Floor((remainingDays - HIJRI_CALCULATION_OFFSET) / islamicYearLength);
            int hijriYear = (HIJRI_CYCLE_YEARS * cycle) + yearInCycle;
            // Calculate remaining days for month calculation
            remainingDays -= (int)Math.Floor((yearInCycle * islamicYearLength) + HIJRI_CALCULATION_OFFSET);
            int hijriMonth = (int)Math.Floor((remainingDays + HIJRI_MONTH_OFFSET) / HIJRI_MONTH_APPROXIMATION);
            // Normalize month value
            if (hijriMonth == MAX_MONTH_VALUE)
            {
                hijriMonth = MONTHS_PER_YEAR;
            }
            int hijriDay = remainingDays - (int)Math.Floor((HIJRI_DAY_OFFSET * hijriMonth) - HIJRI_MONTH_APPROXIMATION);
            return new HijriDateComponents(hijriYear, hijriMonth, hijriDay);
        }

        /// <summary>
        /// Looks up the correction table for the provided Modified Julian Date (MJD)
        /// and converts the lookup result into Hijri year/month/day components.
        /// </summary>
        /// <param name="modifiedJulianDate">The Modified Julian Date (MJD) value to correct.</param>
        /// <returns>A <see cref="HijriDateComponents"/> derived from the correction table.</returns>
        private static HijriDateComponents ApplyCorrectionTable(int modifiedJulianDate)
        {
            // Find the appropriate correction index
            int correctionIndex = FindCorrectionIndex(modifiedJulianDate);
            // Calculate lunar number and derive Hijri components
            int lunarNumber = correctionIndex + HIJRI_LUNAR_NUMBER_OFFSET;
            int yearIndex = (int)Math.Floor((lunarNumber - 1) / (double)MONTHS_PER_YEAR);
            int hijriYear = yearIndex + 1;
            int hijriMonth = lunarNumber - (MONTHS_PER_YEAR * yearIndex);
            // Guard against out-of-range correctionIndex values (0.._dateCorrection.Length)
            int tableIndex = correctionIndex - 1;
            if (tableIndex < 0)
            {
                tableIndex = 0;
            }
            else if (tableIndex >= _dateCorrection.Length)
            {
                tableIndex = _dateCorrection.Length - 1;
            }
            int hijriDay = modifiedJulianDate - _dateCorrection[tableIndex] + 1;
            return new HijriDateComponents(hijriYear, hijriMonth, hijriDay);
        }

        /// <summary>
        /// Finds the smallest index in the <see cref="_dateCorrection"/> table
        /// whose value is greater than the supplied Modified Julian Date. Used
        /// to determine the lunar month mapping for a given MJD.
        /// </summary>
        /// <param name="modifiedJulianDate">The Modified Julian Date to locate in the correction table.</param>
        /// <returns>An index into <see cref="_dateCorrection"/>. If the value is beyond the last entry, returns the table length.</returns>
        private static int FindCorrectionIndex(int modifiedJulianDate)
        {
            for (int index = 0; index < _dateCorrection.Length; index++)
            {
                if (_dateCorrection[index] > modifiedJulianDate)
                {
                    return index;
                }
            }
            return _dateCorrection.Length;
        }

        /// <summary>
        /// Chooses between the correction-table based Hijri date values and the
        /// primary astronomical estimate. The correction-table result is preferred
        /// when its day value looks valid; otherwise the computed estimate is used.
        /// </summary>
        /// <param name="corrected">Hijri date components obtained from the correction table.</param>
        /// <param name="primary">Hijri date components obtained from the primary astronomical algorithm.</param>
        /// <returns>A <see cref="HijriDate"/> instance representing the chosen Hijri date.</returns>
        private static HijriDate SelectMostAccurateHijriDate(HijriDateComponents corrected, HijriDateComponents primary)
        {
            // Use corrected values if day is valid (2 digits or less), otherwise use primary calculation
            bool isCorrectedValid = corrected.Day.ToString(System.Globalization.CultureInfo.InvariantCulture).Length <= MAX_DAY_DIGITS;
            return isCorrectedValid
                ? new HijriDate { Year = corrected.Year, Month = corrected.Month, Date = corrected.Day }
                : new HijriDate { Year = primary.Year, Month = primary.Month, Date = primary.Day };
        }

        #endregion

        #region Private Helper Methods - Hijri to Gregorian Conversion

        /// <summary>
        /// Computes the lunar month sequence number (months since the Hijri epoch)
        /// for the supplied Hijri date. This value is used to index the correction table.
        /// </summary>
        /// <param name="hijriDate">The Hijri date used to compute the lunar month sequence number (months since the Hijri epoch).</param>
        /// <returns>The lunar month count since the Hijri epoch.</returns>
        private static int CalculateLunarNumberFromHijri(HijriDate hijriDate)
        {
            int yearsSinceEpoch = hijriDate.Year - 1;
            return (yearsSinceEpoch * MONTHS_PER_YEAR) + 1 + (hijriDate.Month - 1);
        }

        /// <summary>
        /// Converts a Hijri date to the corresponding Modified Julian Day Number (MJD)
        /// by locating the correction table entry for the lunar number and adding the day offset.
        /// </summary>
        /// <param name="hijriDate">The Hijri date to convert to the corresponding Modified Julian Day (MJD).</param>
        /// <param name="lunarNumber">The lunar number corresponding to the Hijri month (from epoch).</param>
        /// <returns>The Modified Julian Day Number for the given Hijri date.</returns>
        private static int CalculateModifiedJulianDayNumber(HijriDate hijriDate, int lunarNumber)
        {
            int correctionIndex = lunarNumber - HIJRI_LUNAR_NUMBER_OFFSET;
            int tableIndex = correctionIndex - 1;
            if (tableIndex < 0)
            {
                tableIndex = 0;
            ...
            return hijriDate.Date + _dateCorrection[tableIndex] - 1;
        }

        /// <summary>
        /// Converts a Julian Day Number into a Gregorian <see cref="DateTime"/>,
        /// applying the Gregorian correction and extracting year/month/day.
        /// </summary>
        /// <param name="julianDate">The Julian Date number to convert to Gregorian.</param>
        /// <returns>A <see cref="DateTime"/> representing the Gregorian calendar date for the provided Julian Date.</returns>
        private static DateTime ConvertJulianToGregorian(int julianDate)
        {
            int adjustedJulianDate = (int)Math.Floor(julianDate + 0.5);
            // Apply Gregorian calendar correction
            int gregorianCorrection = (int)Math.Floor((adjustedJulianDate - JULIAN_EPOCH_OFFSET) / JULIAN_CENTURY_DAYS);
            int correctedDate = adjustedJulianDate + 1 + gregorianCorrection - (int)Math.Floor(gregorianCorrection / 4.0);
            // Calculate date components
            int baseValue = correctedDate + JULIAN_CALENDAR_ADJUSTMENT;
            int yearCycles = (int)Math.Floor((baseValue - GREGORIAN_CALENDAR_BASE - 0.1) / GREGORIAN_YEAR_DAYS);
            int daysInCycles = (int)Math.Floor(GREGORIAN_YEAR_DAYS * yearCycles);
            int monthApprox = (int)Math.Floor((baseValue - daysInCycles) / MONTH_APPROXIMATION);
            int gregorianDay = baseValue - daysInCycles - (int)Math.Floor(monthApprox * MONTH_APPROXIMATION);
            int gregorianMonth = monthApprox - (monthApprox > 13.5 ? MAX_MONTH_VALUE : 1);
            int gregorianYear = yearCycles - (gregorianMonth > 2.5 ? YEAR_BASE_OFFSET : YEAR_BASE_OFFSET - 1);
            // Adjust for the absence of year zero in the Gregorian calendar
            if (gregorianYear <= 0)
            {
                gregorianYear--;
            }
        ...
        /// <summary>
        /// Internal structure to hold Hijri date components during calculation.
        /// </summary>
        private readonly struct HijriDateComponents
        {
            /// <summary>
            /// Gets the Hijri year computed by the helper methods.
            /// </summary>
            internal int Year { get; }
            /// <summary>
            /// Gets the Hijri month computed by the helper methods.
            /// </summary>
            internal int Month { get; }
            /// <summary>
            /// Gets the Hijri day computed by the helper methods.
            /// </summary>
            internal int Day { get; }

            internal HijriDateComponents(int year, int month, int day)
            {
                Year = year;
                Month = month;
                Day = day;
