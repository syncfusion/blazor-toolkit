    ...
    public class StackingColumnSeriesRenderer : ColumnBaseRenderer
    {
        ...
        internal void CalculateColumnPathOption()
        {
            ...
            foreach (Point pointColumn in visiblePoints)
            {
                ...
                if (pointColumn.Visible && ChartHelper.WithInRange(previousPoint, pointColumn, nextPoint ?? null!, Series?.Renderer.XAxisRenderer ?? null!) && ChartHelper.IsCurrentPointWithinVisibleRange(pointColumn, Series?.Renderer.XAxisRenderer ?? null!) && Series?.Renderer.StackedValues is not null)
                {
